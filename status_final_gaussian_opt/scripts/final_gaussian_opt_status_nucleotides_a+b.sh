...
if [ "$sugar_ring" == "none" ]; then
echo ""$environment"/"$type_sugar"/"$IL"/"$ru" converges" >> $MD_RUN/nucleotides_a+b/final_gaussian_opt/calc_that_did_converged_"$environment".log
else
echo ""$environment"/"$type_sugar"/"$sugar_ring"/"$IL"/"$ru" converges" >> $MD_RUN/nucleotides_a+b/final_gaussian_opt/calc_that_did_converged_"$environment".log
fi

# Determine if the optimized geometry is a min or TS of the PES
if [ "$freq_number" == "NImag=0" ]; then

echo "TC "$type_sugar" with "$sugar_ring" sugar ring conformation, "$IL" and RU "$ru" in "$environment" has NO Imaginary freq ($freq_number)" >> $MD_RUN/nucleotides_a+b/final_gaussian_opt/calc_with_no_imag_freq.log  

else # The case that there are imaginary frequencies

if [ "$sugar_ring" == "none" ]; then 
echo ""$environment"/"$type_sugar"/"$IL"/"$ru" has imaginary freq, #imag freq=($freq_number)" >> $MD_RUN/nucleotides_a+b/final_gaussian_opt/calc_with_imag_freq_"$environment".log  
else
echo ""$environment"/"$type_sugar"/"$sugar_ring"/"$IL"/"$ru" has imaginary freq, #imag freq=($freq_number)" >> $MD_RUN/nucleotides_a+b/final_gaussian_opt/calc_with_imag_freq_"$environment".log
fi
...
if [ "$sugar_ring" == "none" ]; then
echo ""$environment"/"$type_sugar"/"$IL"/"$ru" did not converges" >> $MD_RUN/nucleotides_a+b/final_gaussian_opt/calc_that_did_not_converged_"$environment".log
...
echo ""$environment"/"$type_sugar"/"$sugar_ring"/"$IL"/"$ru" did not converges" >> $MD_RUN/nucleotides_a+b/final_gaussian_opt/calc_that_did_not_converged_"$environment".log
# If the calculation does not converges then it is necesary to Convert log file to gjf
...
sed -i 's/g16 '$type_sugar'_'$ru'_'$IL'_min.gjf/g16 '$type_sugar'_'$ru'_'$IL'_min2.gjf/g' job1.sh
else
# Replace chk file in new gjf 2min file
sed -i 's/%chk=2deoxyribo_beta_vacuum_cell21.chk/%chk='$type_sugar'_'$sugar_ring'_'$ru'_'$IL'_min.chk/g' nucleotide_2opt.gjf
# Replace name in job1.sh file
sed -i 's/g16 '$type_sugar'_'$sugar_ring'_'$ru'_'$IL'_min.gjf/g16 '$type_sugar'_'$sugar_ring'_'$ru'_'$IL'_min2.gjf/g' job1.sh
fi
...
if [ "$sugar_ring" == "none" ]; then
mv nucleotide_2opt.gjf "$type_sugar"_"$ru"_"$IL"_min2.gjf
else
mv nucleotide_2opt.gjf "$type_sugar"_"$sugar_ring"_"$ru"_"$IL"_min2.gjf
fi
