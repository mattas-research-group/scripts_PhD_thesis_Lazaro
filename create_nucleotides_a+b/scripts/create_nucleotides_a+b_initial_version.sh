...
if [[ "$atom" -gt "$hydrogen" ]]; then
new_atom=$(($atom-1))
sed -i 's/'$atom'/'$new_atom'/' column_"$column"_from_sugar_sin_H.txt
fi
...
elif [ "$type_sugar" == "2deoxy_ribopyranose" ] || [ "$type_sugar" == "ribopyranose" ] || [ "$type_sugar" == "threose" ] || [ "$type_sugar" == "glycerol" ] || [ "$type_sugar" == "glyceric_acid" ]; then
...
sed -i 's/G16 phosphate_vacuum_scan.gjf/g16 '$backbone'_scan.gjf/g' job1.sh
...
cat $MD_RUN/list_folders/IL_list.txt | while read IL
...
# Message
echo "working on $environment environment, $type_sugar with $sugar_ring sugar ring conformation and IL $IL" 

# Create the TC-IL backbone
rm *.gjf
create_TC_IL_backbone

# Create list of files to run in Cedar
echo "/initial_gaussian_opt/$environment/$type_sugar/$sugar_ring/$ru/"$backbone"_scan.gjf" >> $MD_RUN/list_TC_IL_backbone.txt
...
else # This is the case that the TC is glycerol or glyceric acid, which is not a sugar and does not has sugar ring conformation
...
# Create list of files to run in Cedar
echo "/initial_gaussian_opt/$environment/$type_sugar/$IL/"$backbone"_scan.gjf" >> $MD_RUN/list_nucleosides.txt
