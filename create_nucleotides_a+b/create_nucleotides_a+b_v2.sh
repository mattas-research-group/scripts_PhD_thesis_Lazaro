...
function gen_nucleotides_ab_path {
...
#read -p "insert RU (e.g. adenine)" RU
...
tc="$type_sugar"_"$sugar_ring"_"$RU"
fi
...
# Delete lines in matrix that correspond to the H that is reacting and will be deleted from the z-matrix
sed ''$line_hydrogen','$line_hydrogen'd' $tc.gzmat > sugar_sin_H.gzmat

#### Delete in internal coordinates values lines with values of the distances, angles and dihedrals for the atom deleted in previous step ###
...
gen_nucleotides_ab_path $environment $TC $ring_conf_fura $RU $IL
...
# Run function that generates the nucleotide
gen_nucleotides_ab_path $environment $TC $ring_conf_pyra $RU $IL
...
# Run function that generates the nucleotide
gen_nucleotides_ab_path $environment $TC $ring_conf $RU $IL
