...
# Run script to run gaussian opt for a second time in case that is necessary
min_file="$TC"_"$ring_conf_fura"_"$RU"_"$IL"_2opt.gjf

if [ -f "$min_file" ]; then

# Rerun the second scan to finish the calculation
sbatch job1.sh
...
# Run script to run gaussian opt for a second time in case that is necessary
min_file="$TC"_"$ring_conf_pyra"_"$RU"_"$IL"_2opt.gjf
...
# Run script to run gaussian opt for a second time in case that is necessary
min_file="$TC"_"$RU"_"$IL"_2opt.gjf
