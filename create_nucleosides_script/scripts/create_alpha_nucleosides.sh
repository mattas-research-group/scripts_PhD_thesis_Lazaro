...
if [[ "$ru" -eq "adenine" && "$RU" -eq "guanine" ]]; then
...
sed -i '3 s/.*/\O  '$second_atom_base'  r'$third_atom_base'  '$first_atom_base'  a'$third_atom_base'  '$anomer'  d'$third_atom_base'/' no_int_coord_base.txt
fi
...
echo "a1= 112.17" >> first_int_coord_base.txt
echo "d1= 94.91" >> first_int_coord_base.txt

# Insert a2 d2 and d3 in this file 
sed -i '2 i a2= 130.23' internal_coordinates_values_base.txt
sed -i '3 i d2= 315.40' internal_coordinates_values_base.txt
sed -i '6 i d3= 358.28' internal_coordinates_values_base.txt
...
# Replace the line
if [[ "$ru" -eq "adenine" && "$ru" -eq "guanine" && "$ru" -eq "cytosine" ]]; then
awk 'NR=='$line' {$0="'$value'= 4.26"} 1' internal_coordinates_base_sinH.gzmat > internal_coordinates_base_sinH_v2.gzmat
else
awk 'NR=='$line' {$0="'$value'= 179.97"} 1' internal_coordinates_base_sinH.gzmat > internal_coordinates_base_sinH_v2.gzmat
fi
