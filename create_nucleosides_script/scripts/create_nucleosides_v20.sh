...
elif [ "$type_sugar" == "peptide" ]; then # THIS IS THE OPTION TO WORK ON PEPTIDE NUCLEOSIDES

echo "work now on the peptide"
...
echo "r1= 1.5200" > first_int_coord_base.txt
echo "a1= 112.00" >> first_int_coord_base.txt
echo "d1= 285.21" >> first_int_coord_base.txt

# Insert a2 d2 and d3 for purines
sed -i '2 i a2= 124.27' internal_coordinates_values_base.txt
sed -i '3 i d2=  75.94' internal_coordinates_values_base.txt
sed -i '6 i d3= 256.40' internal_coordinates_values_base.txt
...
cat $MD_RUN/instructions_DFT/instructions_b3lyp.txt $nucleoside.gjf > "$nucleoside"_new.gjf
...
cat "$nucleoside"_new.gjf $MD_RUN/frozen_coordinates/frozen_coord_fura.txt > "$nucleoside"_new2.gjf
mv "$nucleoside"_new2.gjf $nucleoside.gjf
elif [ "$type_sugar" == "threose" ]; then
cat "$nucleoside"_new.gjf $MD_RUN/frozen_coordinates/frozen_coord_threo.txt > "$nucleoside"_new2.gjf
mv "$nucleoside"_new2.gjf $nucleoside.gjf
elif [ "$type_sugar" == "2deoxy_ribopyranose" ] || [ "$type_sugar" == "ribopyranose" ]; then
cat "$nucleoside"_new.gjf $MD_RUN/frozen_coordinates/frozen_coord_pyra.txt > "$nucleoside"_new2.gjf
mv "$nucleoside"_new2.gjf $nucleoside.gjf
else
