...
read -p "insert type of sugar in case that it is (e.g. 2deoxy_ribofuranose, or ribofuranose, or 2deoxy_ribopyranose, or ribopyranose, or threose, or glycerol)" type_sugar
read -p "insert conformation of the sugar ring (e.g. 2endo_alpha, or 2endo_beta, or D1C4_alpha, or D4C1_beta, or none if it is glycerol or glyceric acid)" sugar_ring
...
# Convert ru to gzmat
if [ "$ru" == "BA_cbond" ] || [ "$ru" == "BA_nbond" ] || [ "$ru" == "TARC_cbond" ] || [ "$ru" == "TARC_nbond" ] || [ "$ru" == "melamine" ] || [ "$type_sugar" == "peptide" ]; then
...
# Put here the labels of the atoms
anomer=1
hemiacetal_O=2
hydrogen=13
...
if [ "$type_sugar" == "2deoxy_ribofuranose" ] || [ "$type_sugar" == "ribofuranose" ] || [ "$type_sugar" == "glycerol" ]; then
awk 'NR=='$lines' { print $1, $2, $3, $4, $5, $6, $7 }' no_int_coord_base_labels_fixed_v2.txt >> no_int_coord_base_labels_fixed_v3.txt
...
else ## THIS IS THE PART TO WORK ON PEPTIDE NUCLEIC ACIDS

##########################
## WORK ON PEPTIDE FILE ##
##########################

atom_before_hemiacetal=$(($hemiacetal_O+1))

# Obtain the line number for hydrogen to be deleted
line_hydrogen=$(($hydrogen+6))

# Delete lines in matrix that correspond to the H atom
sed ''$line_hydrogen','$line_hydrogen'd' $tc.gzmat > peptide_sin_H.gzmat

#### Delete lines with values of the distances, angles and dihedrals for the atoms deleted in previous step
sed -i '/r'$hydrogen'/d' peptide_sin_H.gzmat
sed -i '/a'$hydrogen'/d' peptide_sin_H.gzmat
sed -i '/d'$hydrogen'/d' peptide_sin_H.gzmat

########### Calculate total number of atoms in peptide ############

# Delete first headings of this file
sed '1,6d' $tc.gzmat > sinheadings_peptide.gzmat

# Delete values the internal coordinates
sed -i '1,/Variables:/!d' sinheadings_peptide.gzmat
sed -i '$d' sinheadings_peptide.gzmat

# Count number of lines in this file
< sinheadings_peptide.gzmat wc -l > number_atoms_peptide.txt

atoms_sugar=`cat number_atoms_peptide.txt`
atoms_sugar_ion=$(($atoms_sugar-1))
lines_plus_atoms_in_sugar_ion=$(($atoms_sugar_ion+6))
starting_line_int_coord=$(($atoms_sugar_ion+7))

# Copy lines with values of internal coordinates to another file
sed -n ''$starting_line_int_coord',$p' peptide_sin_H.gzmat > internal_coordinates_values_peptide_sin_H.txt 
# Eliminate last empty line
sed -i '$ d' internal_coordinates_values_peptide_sin_H.txt   

# Delete values the internal coordinates
sed -i '1,/Variables:/!d' peptide_sin_H.gzmat
sed -i '$d' peptide_sin_H.gzmat
...
new_label=$(($lines-6))
# Substitute old lines by new ones
sed -i ''$lines's/r'$old_label'/r'$new_label'/' peptide_sin_H.gzmat
sed -i ''$lines's/a'$old_label'/a'$new_label'/' peptide_sin_H.gzmat
sed -i ''$lines's/d'$old_label'/d'$new_label'/' peptide_sin_H.gzmat

fi
done

################## WORK ON INTERNAL COORDINATES VALUES FILE FOR PEPTIDE ##################
# Work on correcting the rlabel, alabel and dlabel in the file with internal coordinates values 
for labels in $(seq 2 1 $atoms_sugar_ion)
...
# Remove heading
sed -i '1,1d' internal_coordinates_values_peptide_sin_H.txt

# Eliminate everything before the = sign in old int coord file from the base and eliminate first line
cut -f2 -d"=" internal_coordinates_values_peptide_sin_H.txt > internal_coordinates_values_peptide_bef_nucleo.txt

# Paste new labels and values for internal coordinates
paste labels_sugar_fix.txt internal_coordinates_values_peptide_bef_nucleo.txt > new_internal_coordinates_peptide_bef_nucleo.txt

# Reduce spaces between two columns
sed -i $'s/\t//g' new_internal_coordinates_peptide_bef_nucleo.txt
tr -s " " < new_internal_coordinates_peptide_bef_nucleo.txt > internal_coordinates_peptide_sinH_before.gzmat

# Delete previous file
rm internal_coordinates_values_peptide_sin_H.txt

# Add heading to internal coordinates values
sed -i '1 i Variables:' internal_coordinates_peptide_sinH_before.gzmat
cp internal_coordinates_peptide_sinH_before.gzmat internal_coordinates_values_peptide_sinH.txt ### THIS IS THE FINAL FILE FOR INTERNAL COORDINATES VALUES OF THE PEPTIDE

# Change the name of the z matrix file for peptide without H atom
mv peptide_sin_H.gzmat no_int_coord_peptide_sinH.gzmat ### THIS IS THE FINAL FILE FOR THE Z MATRIX OF PEPTIDE WITHOUT THE INT COORD VALUES
...
# Delete last two lines of this file that correspond to the hydroxil group deleted from the carboxylate 
sed ''$oxygen','$hydrogen'd' no_int_coord_base.txt
...
# Replace third line with another specific coordinates
sed -i '3 s/.*/\'$third_symbol'  '$first_atom_base'  r'$third_atom_base'  '$anomer'  a'$third_atom_base'  '$hemiacetal_O'  d'$third_atom_base'/' no_int_coord_base.txt
...
# Fine lines of last two hydrogens in metilene group of the carboxylate
hydrogen_met1=$(($atoms_base-5))
hydrogen_met2=$(($atoms_base-4))

# Replace other d#, a# or r# in z matrix of the acetylated base
for lines in $(seq 1 1 $ions_atoms_base)

do

#if [ "$ru" == "adenine_ac" ] || [ "$ru" == "guanine_ac" ]; then

if [ "$lines" == 4 ] || [ "$lines" == "$hydrogen_met1" ] || [ "$lines" == "$hydrogen_met2" ]; then
awk 'NR=='$lines' { print $1, $2, $3, $4, $5, $6='$anomer', $7 }' no_int_coord_base_labels_fixed_v2.txt >> no_int_coord_base_labels_fixed_v3.txt
else
awk 'NR=='$lines' { print $1, $2, $3, $4, $5, $6, $7 }' no_int_coord_base_labels_fixed_v2.txt >> no_int_coord_base_labels_fixed_v3.txt
fi

#elif [ "$ru" == "cytosine_ac" ]; then
...
# Rename this file as final
mv no_int_coord_base_labels_fixed_v3.txt no_int_coord_acetylated_base_sinOH.gzmat  #### THIS IS THE FINAL FILE FOR THE Z MATRIZ OF THE BASE FIXED APPROPIATELY
...
# Delete line of oxygen and hydrogen that has to be delete from carboxylate group of acetylated base
sed -i '/r'$oxygen'/d' internal_coordinates_values_base.txt
sed -i '/a'$oxygen'/d' internal_coordinates_values_base.txt
sed -i '/d'$oxygen'/d' internal_coordinates_values_base.txt
sed -i '/r'$hydrogen'/d' internal_coordinates_values_base.txt
sed -i '/a'$hydrogen'/d' internal_coordinates_values_base.txt
sed -i '/d'$hydrogen'/d' internal_coordinates_values_base.txt
...
# Insert a1 and d1 for purines
echo "r1= 1.5200" > first_int_coord_base.txt
echo "a1= 112.00" >> first_int_coord_base.txt
echo "d1=  63.20" >> first_int_coord_base.txt

# Insert a2 d2 and d3 for purines
sed -i '2 i a2= 124.27' internal_coordinates_values_base.txt
sed -i '3 i d2=  60.74' internal_coordinates_values_base.txt
sed -i '6 i d3= 242.90' internal_coordinates_values_base.txt

# Replace other values of angles
sed -i '9s/.*/d4= 186.35/' internal_coordinates_values_base.txt

# Obtain lines of two methylene hydrogens and replace their values
line_number_int_coord_Hm1=$((($ions_atoms_base*3)-3-9))
line_number_int_coord_Hm2=$((($ions_atoms_base*3)-3-6))
#hydrogen_m1_new_label=$(($hydrogen_met1+$atoms_sugar_ion))
#hydrogen_m2_new_label=$(($hydrogen_met2+$atoms_sugar_ion))
sed -i ''$line_number_int_coord_Hm1's/.*/d'$hydrogen_met1'=  63.37/' internal_coordinates_values_base.txt
sed -i ''$line_number_int_coord_Hm2's/.*/d'$hydrogen_met2'= 305.96/' internal_coordinates_values_base.txt
...
tr -s " " < new_internal_coordinates_base2.txt > internal_coordinates_acetylated_base_sinOH.gzmat #### THIS IS THE FINAL FILE WITH THE INTERNAL COORD VALUES FIXED FOR THE ACETYLATED BASE 

################### Now paste all together to create final z matriz file for the nucleosides ######################
cat no_int_coord_peptide_sinH.gzmat no_int_coord_acetylated_base_sinOH.gzmat internal_coordinates_values_peptide_sinH.txt internal_coordinates_acetylated_base_sinOH.gzmat > nucleoside.gzmat
...
mv "$nucleoside"_new.gjf $nucleoside.gjf
fi
