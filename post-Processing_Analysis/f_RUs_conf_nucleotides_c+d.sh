...
#	f_RUs_conf_nucleotides_c+d.sh
#
# 18-jun-2023	Modify options changing bold of y axis for circular plot
# 05-Mar-2023	Created
...
# ./gen_RUs_conf_csv_graphs_files_nucleo_a+b_v1.sh
...
# Obtain number of atoms in the backbone
nAtoms_backbone=`cat n_atoms_backbone.txt`
...
if [ "$RU" == "adenine" ]; then
a1_base=$((1+$nAtoms_backbone))
a2_base=$((9+$nAtoms_backbone))

elif [ "$RU" == "guanine" ]; then
a1_base=$((1+$nAtoms_backbone))
a2_base=$((11+$nAtoms_backbone))

elif [ "$RU" == "cytosine" ] || [ "$RU" == "thymine" ] || [ "$RU" == "uracil" ] || [ "$RU" == "CA" ] || [ "$RU" == "melamine" ]; then
a1_base=$((1+$nAtoms_backbone))
a2_base=$((2+$nAtoms_backbone))
...
echo $X_value >> $MD_RUN/nucleotides_c+d/post_proc_summary_results_csv_graphs/RU_conformation/$IL/$TC/chi_values.txt
echo $position >> $MD_RUN/nucleotides_c+d/post_proc_summary_results_csv_graphs/RU_conformation/$IL/$TC/TC_RU_relative_orientation.txt

echo $environment >> $MD_RUN/nucleotides_c+d/post_proc_summary_results_csv_graphs/RU_conformation/$IL/$TC/environment_list_names.txt
echo $TC >> $MD_RUN/nucleotides_c+d/post_proc_summary_results_csv_graphs/RU_conformation/$IL/$TC/TC_list_names.txt
echo $ring_conf >> $MD_RUN/nucleotides_c+d/post_proc_summary_results_csv_graphs/RU_conformation/$IL/$TC/ring_conf_list_names.txt
echo $RU >> $MD_RUN/nucleotides_c+d/post_proc_summary_results_csv_graphs/RU_conformation/$IL/$TC/RU_list_names.txt
echo $IL >> $MD_RUN/nucleotides_c+d/post_proc_summary_results_csv_graphs/RU_conformation/$IL/$TC/IL_list_names.txt
...
mkdir nucleotides_c+d/post_proc_summary_results_csv_graphs/RU_conformation

# Create IL folder inside the post-processing folder
for IL in "${IL_names[@]}"; do
mkdir nucleotides_c+d/post_proc_summary_results_csv_graphs/RU_conformation/$IL

# Create TC folder inside the post-processing folder
for TC in "${TC_names[@]}"; do
mkdir nucleotides_c+d/post_proc_summary_results_csv_graphs/RU_conformation/$IL/$TC
done
done
...
for TC in TC_names:
    ...
    for i, (ax, count_alpha, count_beta, mean, std) in enumerate([(axn.flat[0], info_can_vacuum[0], info_can_vacuum[1], 
        ...
        # Change titles and legend configuration
        if i == 0:
    ...
    plt.tight_layout()       
    plt.savefig('Rose_diagram_'+str(TC[1])+'_'+str(IL[1])+'_c+d.pdf')                                                  
