        ...
        if env == "vacuum" and type_ru == "canonical":
            if TC in ['2deoxy_ribofuranose', 'ribofuranose', 'threose', '2deoxy_ribopyranose', 'ribopyranose']:
                ...
                ax1 = ru_df.plot(kind='bar', ax=ax, color='Grey', rot=360, width=0.8, edgecolor = "black", 
                                 xlabel='', ylabel='ΔG (kJ/mol)', legend=False, fontsize=28)
            ax1.set_xticks([]) 
            ax1.set_ylabel(str(thermo)+ " (kJ/mol)", fontsize=28, fontdict=dict(weight='bold'), rotation=90)
            ...
            if TC in ['2deoxy_ribofuranose', 'ribofuranose', 'threose', '2deoxy_ribopyranose', 'ribopyranose']:
                ...
                ax3 = ru_df.plot(kind='bar', ax=ax, color='Grey', rot=360, width=0.8, edgecolor = "black", 
                                 xlabel='', ylabel='ΔG (kJ/mol)', legend=False, fontsize=28)
            ax3.set_ylabel(str(thermo) + " (kJ/mol)", fontsize=28, fontdict=dict(weight='bold'), rotation=90)
            #ax3.set_title('Water canonical RUs', weight='bold', fontsize=28)
            ...
            if TC in ['2deoxy_ribofuranose', 'ribofuranose', 'threose']:
                ...
                ax4.legend(['α-$^1C_4$', 'β-$^1C_4$', 'α-$^4C_1$', 'β-$^4C_1$'], loc='center left', bbox_to_anchor=(1, 0.5), fontsize=24)
            #else:
