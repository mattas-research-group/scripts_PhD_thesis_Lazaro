...
#	f_CPpuckering_nucleotides_c+d.sh

#	19-jun-2023	Modify function to generate graphs. 
# 	05-Mar-2023	Created.

# Description:
# This script calculates the generalized 
# puckering parameters from Cremmer & Pople.
#
# Graphs:
# For furanoses generates rose diagrams for the 
# phase angle.
# For pyranoses generates heat maps for the phase
# angles for the pyranoses and freq histograms for 
# the total puckering amplitude.
#
# Definitions:
# Furanoses:
# phase angle: phi2
#
# Pyranoses:
# Phase angle#1: phi2 (degrees)
# Phase angle#2: theta (degrees)
# Total puckering amplitude: Q (Amstrongs)
#
# To run script type:
# chmod u+x f_CPpuckering_nucleotides_c+d.sh 
# ./f_CPpuckering_nucleotides_c+d.sh
...
echo "$ring_conformation_C_P" >> $MD_RUN/nucleotides_c+d/post_proc_summary_results_csv_graphs/sugar_puckering/$IL/$TC/final_ring_conformation_C_P_furanoses.txt
echo "$q2_amplitude_C_P" >> $MD_RUN/nucleotides_c+d/post_proc_summary_results_csv_graphs/sugar_puckering/$IL/$TC/q2_C_P_furanoses.txt
echo "$total_amplitude_Q_C_P" >> $MD_RUN/nucleotides_c+d/post_proc_summary_results_csv_graphs/sugar_puckering/$IL/$TC/total_amplitude_Q_C_P_furanoses.txt
echo "$phase_angle_phi2_C_P" >> $MD_RUN/nucleotides_c+d/post_proc_summary_results_csv_graphs/sugar_puckering/$IL/$TC/phase_angle_phi2_Q_C_P_furanoses.txt
echo "$RU" >> $MD_RUN/nucleotides_c+d/post_proc_summary_results_csv_graphs/sugar_puckering/$IL/$TC/RU_names_furanoses.txt
echo "$TC" >> $MD_RUN/nucleotides_c+d/post_proc_summary_results_csv_graphs/sugar_puckering/$IL/$TC/TC_names_furanoses.txt
echo "$ring_conf" >> $MD_RUN/nucleotides_c+d/post_proc_summary_results_csv_graphs/sugar_puckering/$IL/$TC/initial_ring_conf_furanoses.txt
echo "$environment" >> $MD_RUN/nucleotides_c+d/post_proc_summary_results_csv_graphs/sugar_puckering/$IL/$TC/environment_list_furanoses.txt
echo "$IL" >> $MD_RUN/nucleotides_c+d/post_proc_summary_results_csv_graphs/sugar_puckering/$IL/$TC/IL_names_furanoses.txt
...
# Output variables in general files inside the environment folder depending on the ring type
echo "$ring_conformation_pyra_C_P" >> $MD_RUN/nucleotides_c+d/post_proc_summary_results_csv_graphs/sugar_puckering/$IL/$TC/final_ring_conformation_C_P_pyranoses.txt
echo "$q2_amplitude_pyra_C_P" >> $MD_RUN/nucleotides_c+d/post_proc_summary_results_csv_graphs/sugar_puckering/$IL/$TC/q2_C_P_pyranoses.txt
echo "$q3_amplitude_pyra_C_P" >> $MD_RUN/nucleotides_c+d/post_proc_summary_results_csv_graphs/sugar_puckering/$IL/$TC/q3_C_P_pyranoses.txt
echo "$total_amplitude_Q_pyra_C_P" >> $MD_RUN/nucleotides_c+d/post_proc_summary_results_csv_graphs/sugar_puckering/$IL/$TC/total_amplitude_Q_C_P_pyranoses.txt
echo "$phase_angle_phi2_pyra_C_P" >> $MD_RUN/nucleotides_c+d/post_proc_summary_results_csv_graphs/sugar_puckering/$IL/$TC/phase_angle_phi2_Q_C_P_pyranoses.txt
echo "$phase_angle_theta_C_P" >> $MD_RUN/nucleotides_c+d/post_proc_summary_results_csv_graphs/sugar_puckering/$IL/$TC/phase_angle_theta_Q_C_P_pyranoses.txt
echo "$RU" >> $MD_RUN/nucleotides_c+d/post_proc_summary_results_csv_graphs/sugar_puckering/$IL/$TC/RU_names_pyranoses.txt
echo "$TC" >> $MD_RUN/nucleotides_c+d/post_proc_summary_results_csv_graphs/sugar_puckering/$IL/$TC/TC_names_pyranoses.txt
echo "$ring_conf" >> $MD_RUN/nucleotides_c+d/post_proc_summary_results_csv_graphs/sugar_puckering/$IL/$TC/initial_ring_conf_pyranoses.txt
echo "$environment" >> $MD_RUN/nucleotides_c+d/post_proc_summary_results_csv_graphs/sugar_puckering/$IL/$TC/environment_list_pyranoses.txt
echo "$IL" >> $MD_RUN/nucleotides_c+d/post_proc_summary_results_csv_graphs/sugar_puckering/$IL/$TC/IL_names_furanoses.txt
...
mkdir nucleotides_c+d/post_proc_summary_results_csv_graphs/sugar_puckering

# Create IL folder inside the post-processing folder
for IL in "${IL_names[@]}"; do
mkdir nucleotides_c+d/post_proc_summary_results_csv_graphs/sugar_puckering/$IL

# Create TC folder inside the post-processing folder
for TC in "${TC_names[@]}"; do
mkdir nucleotides_c+d/post_proc_summary_results_csv_graphs/sugar_puckering/$IL/$TC
done
done
...
w_results_to_analysis_folder $environment $TC $ring_conf_fura $RU $IL
...
# Write results to general files in analysis folder
w_results_to_analysis_folder $environment $TC $ring_conf_pyra $RU $IL
...
if TC in ["2deoxy_ribofuranose", "ribofuranose", "threose"]:
    ...
    plt.tight_layout()    
    plt.savefig('rose_diagram_'+str(TC)+'_'+str(IL)+'_a+b.pdf')      
    ...
    fig, axn = plt.subplots(2, 3, sharey=False, sharex=False, figsize=(36,22))
    ...
    for i, (ax, phi2, theta, environment, phi2_alpha, phi2_beta, theta_alpha, theta_beta) in enumerate([(axn.flat[0], 
        ...
        if ax == axn.flat[4] or ax == axn.flat[5]:        
            ax.set_xlabel("\n" + r'$\varphi_2$' + "(°)", fontsize = 38, fontweight='bold')
        elif ax == axn.flat[0]:
        ...
        if ax == axn.flat[5]:
            ...
            ax.legend(handles=[grey_square, green_triangle], fontsize=40, bbox_to_anchor=(1.2, -0.23), loc="lower center", ncol=2)
    ...
    for i, (ax, phi2_alpha, phi2_beta, theta_alpha, theta_beta, environment) in enumerate([(axn.flat[0], 
        ...
        # Add titles
        if ax == axn.flat[0]:
            ax.set_title('Canonical, vacuum\n', fontweight="bold", fontsize=40)
        elif ax == axn.flat[1]:
            ax.set_title('Non_canonical, vacuum\n', fontweight="bold", fontsize=40)
        elif ax == axn.flat[2]:
        ...
        # Set legend
        if ax == axn.flat[5]:
            ax.legend(fontsize=27, bbox_to_anchor=(1.2, -0.2), loc="lower center", ncol=2)
                
    plt.tight_layout(h_pad=1)
    plt.savefig('Scatter_plot_'+str(TC)+'_'+str(IL)+'_a+b.pdf')
    ...
    plt.tight_layout()
    plt.savefig('Q_freq_hist_'+str(TC)+'_'+str(IL)+'_a+b.pdf')  



################# Merge puckering params in one cell for tables in paper  
...
# Round params to 1 decimal       
v = df[cols].round(1)

df[cols] = v
if TC == "2deoxy_ribofuranose" or TC == "ribofuranose" or TC == "threose":
    df['combined_q2_Q'] = df[cols].apply(lambda row: '\n'.join(row.values.astype(str)), axis=1) 
else:
    df['combined_q2_theta_Q'] = df[cols].apply(lambda row: '\n'.join(row.values.astype(str)), axis=1) 
    
df.to_csv('df_with_CPpucker_values_to_copy.csv')   
