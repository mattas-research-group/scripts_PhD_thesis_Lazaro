...
unique_opt_file="$tc"_min
second_opt_file="$tc"_min2
third_opt_file="$tc"_min3
...
# Delete last line corresponding to H atom deleted to create z matrix for ionic base: same as delete hydrogen from base to create ion 
...
if [ "$type_sugar" == "glycerol" ] || [ "$type_sugar" == "glyceric_acid" ]; then
nucleotide=""$type_sugar"_"$ru"_"$IL"_scan"
else
nucleotide=""$type_sugar"_"$sugar_ring"_"$ru"_"$IL"_scan"
fi

# Convert this file to gjf
obabel nucleoside.gzmat -O $nucleotide.mol

############################################### Adjust torsion angle of glycosidic bond ################################################
# Convert mol file to gzmat
obabel $nucleotide.mol -O $nucleotide.gzmat
...
sed -i '6s/.*/-1 1/' nucleotide.gzmat

# Convert final gzmat file to gjf file
obabel $nucleotide.gzmat -O "$nucleotide"_new.gjf

# Paste instructions for b3lyp
sed -i -e 1,2d "$nucleotide"_new.gjf
cat $MD_RUN/instructions_DFT/instructions_b3lyp.txt "$nucleotide"_new.gjf > "$nucleotide".gjf

# Substitute keywords for the case of water opt
if [ "$environment" == "water" ]; then
sed -i 's/scf=maxcycle=1600/scf=maxcycle=1600 scrf=(solvent=water)/g' "$nucleotide".gjf
fi

# Replace name chk by name of cell
sed -i 's/%chk=2deoxyribo_beta_vacuum_cell21.chk/%chk='$nucleotide'.chk/g' "$nucleotide".gjf

# Add coordinates to scan 
echo "* "$anomer" "$first_atom_base" *" >> "$nucleotide".gjf 
echo "D "$hemiacetal_O" "$anomer" "$first_atom_base" "$second_atom_base" S 6 60.000000" >> "$nucleotide".gjf

# Delete unnecesary files
rm "$nucleotide"_new.gjf
#rm *.txt
#rm *.gzmat
...
sed -i 's/g16 input.gjf/g16 '$nucleotide'.gjf/g' job1.sh
...
create_nucleotide_cd $environment $TC $ring_conf_fura $RU $IL 
...
create_nucleotide_cd $environment $TC $ring_conf_pyra $RU $IL
...
# Time to copy the corresponding files from the nucleosides folders and the IL folders
create_nucleotide_cd $TC $ring_conf $RU $IL
