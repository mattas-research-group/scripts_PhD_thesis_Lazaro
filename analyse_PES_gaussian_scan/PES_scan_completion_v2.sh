...
# Decide if the folder contains only one or more calculations
if [ "$sugar_ring" == "none" ]; then
unique_scan_file="$type_sugar"_"$ru"_"$IL"_scan.log
second_scan_file="$type_sugar"_"$ru"_"$IL"_scan2.log
else
unique_scan_file="$type_sugar"_"$sugar_ring"_"$ru"_"$IL"_scan.log
second_scan_file="$type_sugar"_"$sugar_ring"_"$ru"_"$IL"_scan2.log
fi

if [ -f "$second_scan_file" ]; then
log_file=$second_scan_file
echo "there is a second scan file"
echo "the scan file is $log_file"
else
log_file=$unique_scan_file
fi
...
if [ "$sugar_ring" == "none" ]; then
tail -n +9 "$type_sugar"_"$ru"_"$IL"_scan.gjf > atoms_symbols.txt
else
tail -n +9 "$type_sugar"_"$sugar_ring"_"$ru"_"$IL"_scan.gjf > atoms_symbols.txt
fi
...
# Find if there is a line that says calculation could not be archived
grep 'This type of calculation cannot be archived.' $log_file > message_about_archived_data.txt
